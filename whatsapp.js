...
const isWebhookEventAllowed = (type) => {
    const allowedEvents = process.env.APP_WEBHOOK_ALLOWED_EVENTS || 'ALL'
...
const sendLaravelWebhook = async (sessionId, type, data) => {
    const appUrl = getAppUrl()

    if (!appUrl || !isWebhookEventAllowed(type)) {
        return
    }

    try {
        await axios.post(`${appUrl}/api/send-webhook/${sessionId}`, {
            type,
            data,
        })
    } catch (error) {
        console.error('Unable to forward webhook event to Laravel.', {
            sessionId,
            type,
...
const serializeConnectionUpdate = (update) => {
...
const createSession = async (sessionId, res = null, options = { usePairingCode: false, phoneNumber: '' }) => {
    ...
    wa.ev.on('messages.upsert', async (m) => {
        ...
        if (messages.length > 0) {
            ...
            void sendLaravelWebhook(sessionId, 'MESSAGES_UPSERT', messageTmp)
        }
    })
    ...
    wa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open' || connection === 'close') {
            void sendLaravelWebhook(sessionId, 'CONNECTION_UPDATE', serializeConnectionUpdate(update))
        }
